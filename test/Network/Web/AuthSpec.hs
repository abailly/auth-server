...
spec = around startStopServer $
  describe "Authentication Server" $ do
    ...
    it "can register user/password given a valid registration token" $ \(getServerPort -> authServerPort, mgr) -> do
      validRegistrationToken <- signedTokenFor claims
...
signedTokenFor :: AuthenticatedUser -> IO LBS.ByteString
signedTokenFor claims = either (error . show) id <$> makeJWT claims (defaultJWTSettings sampleKey) Nothing
