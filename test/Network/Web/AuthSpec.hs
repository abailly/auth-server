...
spec = parallel $
  around startStopServer $
    describe "Authentication Server" $ do
      ...
      it "can retrieve a token given valid credentials" $ \(getServerPort -> authServerPort, mgr) -> do
      ...
      it "cannot retrieve a token given invalid credentials" $ \(getServerPort -> authServerPort, mgr) -> do
